            ...
            fetch('/api/rides')
                ...
                .then(data => {
                    ...
                    if (!data.hammerhead_configured && !data.hammerhead_linked) {
                        ...
                        const authUrl = 'https://api.hammerhead.io/v1/auth/oauth/authorize?client_id=' + encodeURIComponent(data.client_id) + '&redirect_uri=' + encodeURIComponent('http://localhost:8080/callback') + '&response_type=code&scope=activities&state=directeur';
